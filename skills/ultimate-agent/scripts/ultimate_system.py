...
class UltimateAgentSystem:
    ...
    def __init__(self, workspace: Optional[Path] = None):
        ...
        # 使用技能目录下的 memory（__file__ 在 scripts 目录，需要往上两层）
...
class ProactiveEngine:
    ...
    def _check_memory_files(self):
        ...
        # 只检查状态文件是否存在，不检查目录（因为技能自带 memory 目录）
        state_file = memory_dir / "ultimate-state.json"
        if not state_file.exists():
            issues.append({
                "type": "state_missing",
                "description": "系统状态文件不存在，可能需要初始化",
                "priority": "medium"
            })
...
class AgentFactory:
    ...
    def analyze_needs(self):
        ...
        for project in active_projects:
            ...
            # 新增：AI 分身集群
            if "multi-agent" in tags or "orchestration" in tags:
                needs.append({
                    "name": "agent-coordinator",
                    "description": "多代理协调器 - 管理和调度多个 specialized agents",
                    "capabilities": ["orchestrate", "delegate", "synthesize"],
                    "confidence": 0.9
                })
            
            # 新增：全平台消息聚合
            if "messaging" in tags or "integration" in tags:
                needs.append({
                    "name": "message-aggregator",
                    "description": "消息聚合代理 - 统一处理微信/WhatsApp/Telegram/Slack 等平台",
                    "capabilities": ["connect", "aggregate", "route"],
                    "confidence": 0.85
                })
            
            # 新增：自动研究助手
            if "research" in tags or "summarization" in tags:
                needs.append({
                    "name": "research-assistant",
                    "description": "自动研究代理 - 搜索、整理、总结主题最新进展",
                    "capabilities": ["search", "analyze", "summarize", "report"],
                    "confidence": 0.9
                })
